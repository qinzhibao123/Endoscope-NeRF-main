...
sys.path.append('../')
import imageio
from config_f import config_parser
from endoscope.sample_ray import RaySamplerSingleImage
from endoscope.render_image import render_single_image
from endoscope.model import Endoscope_NeRFModel
from utils import *
from endoscope.projection import Projector

from endoscope.data_loaders import dataset_dict
...
if __name__ == '__main__':
    ...
    model = Endoscope_NeRFModel(args, load_scheduler=False, load_opt=False)
    ...
    pred_ph = tf.compat.v1.placeholder(tf.float32)  #tf.placeholder(tf.float32)
    ...
    for i, data in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            ret = render_single_image(ray_sampler=ray_sampler, ray_batch=ray_batch, model=model, projector=projector, chunk_size=args.chunk_size, det=True,
                                      N_samples=args.N_samples, inv_uniform=args.inv_uniform, N_importance=args.N_importance, white_bkgd=args.white_bkgd, featmaps=featmaps)
            ...
            coarse_pred_depth_colored = colorize_np(coarse_pred_depth, range=tuple(data['depth_range'].squeeze().cpu().numpy()))
            ...
            if ret['outputs_fine'] is not None:
                ...
                fine_pred_depth_colored = colorize_np(fine_pred_depth, range=tuple(data['depth_range'].squeeze().cpu().numpy()))
            ...
            print("==================\n"
                  ...
                  "===================\n"
                  .format(scene_name, file_id, coarse_psnr, fine_psnr, running_mean_coarse_psnr, running_mean_fine_psnr, coarse_ssim, fine_ssim,
                          running_mean_coarse_ssim, running_mean_fine_ssim, coarse_lpips, fine_lpips, running_mean_coarse_lpips, running_mean_fine_lpips))

            results_dict[scene_name][file_id] = {'coarse_psnr': coarse_psnr, 'fine_psnr': fine_psnr, 'coarse_ssim': coarse_ssim, 'fine_ssim': fine_ssim,
                                                 'coarse_lpips': coarse_lpips, 'fine_lpips': fine_lpips,}
    ...
    print('------{}-------\n'
          ...
          .format(scene_name, mean_coarse_psnr, mean_fine_psnr, mean_coarse_ssim, mean_fine_ssim, mean_coarse_lpips, mean_fine_lpips,))
