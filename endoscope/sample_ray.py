...
class RaySamplerSingleImage(object):
    def __init__(self, data, device, resize_factor=1, render_stride=1):
        ...
        self.rgb = data['rgb'] if 'rgb' in data.keys() else None  #None
        self.camera = data['camera']
        self.rgb_path = data['rgb_path']
        self.depth_range = data['depth_range']
        self.device = device
        W, H, self.intrinsics, self.c2w_mat = parse_camera(self.camera)  #camra：渲染图像的相机
        self.batch_size = len(self.camera)  #1

        self.H = int(H[0])  #704
        self.W = int(W[0])  #340

        # half-resolution output  半分辨率输出
        if resize_factor != 1:  #不执行
        ...
        self.rays_o, self.rays_d = self.get_rays_single_image(self.H, self.W, self.intrinsics, self.c2w_mat)  #都为(756*1008,3)
        if self.rgb is not None:  #不执行
            self.rgb = self.rgb.reshape(-1, 3)

        if 'src_rgbs' in data.keys():  #执行
            self.src_rgbs = data['src_rgbs']  #(1,10,756,1008,3)
        else:
            self.src_rgbs = None
        if 'src_cameras' in data.keys():  #执行
            self.src_cameras = data['src_cameras']  #(1,10,34)
        else:
