## Slightly modified version of LLFF data loading code,  see https://github.com/Fyusion/LLFF for original
...
def recenter_poses(poses):
    ...
    c2w = np.concatenate([c2w[:3, :4], bottom], -2)  #（3，4）
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False, load_imgs=True):   #bd_factor=0.75, factor=4, basedir:data/nerf_llff_data/fern
    ...
    if recenter:
        poses = recenter_poses(poses)   #(20,3,5)
    ...
    if spherify:
        ...
        ## Get spiral
        # Get average pose
        up = normalize(poses[:, :3, 1].sum(0))   #（3,）
        ...
        tt = poses[:, :3, 3]  #(20,3)    ptstocam(poses[:3,3,:].T, c2w).T
        rads = np.percentile(np.abs(tt), 40, 0)  #(3,)
        c2w_path = c2w #(3,5)
        N_views = 120
        N_rots = 3
        if path_zflat:
    ...
    dists = np.sum(np.square(c2w[:3, 3] - poses[:, :3, 3]), -1)  #(20,)
