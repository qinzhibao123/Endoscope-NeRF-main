...
class LLFFTestDataset(Dataset):
    def __init__(self, args, mode, scenes=(), random_crop=True, **kwargs):
        self.folder_path = os.path.join(args.rootdir, 'data/nerf_llff_data/')  #nerf_llff_data
        self.args = args
        self.mode = mode  # train / test / validation
        self.num_source_views = args.num_source_views  #10
        ...
        all_scenes = os.listdir(self.folder_path)  #['fern']
        if len(scenes) > 0:
        ...
        for i, scene in enumerate(scenes):
            scene_path = os.path.join(self.folder_path, scene)  #data/nerf_llff_data/fern
            _, poses, bds, render_poses, i_test, rgb_files = load_llff_data(scene_path, load_imgs=False, factor=4)
            near_depth = np.min(bds)
            far_depth = np.max(bds)
            intrinsics, c2w_mats = batch_parse_llff_poses(poses)

            i_test = np.arange(poses.shape[0])[::self.args.llffhold]  #(3,)  [0,8,16]   llffhold=8，测试集中每个8个取一个作为测试集
            i_train = np.array([j for j in np.arange(int(poses.shape[0])) if
                                (j not in i_test and j not in i_test)])  #(17,)   剩下的作为训练集

            if mode == 'train':
                i_render = i_train
            else:
                i_render = i_test   #3个测试集的视角作为渲染视角， 用于和训练集做对比

            self.train_intrinsics.append(intrinsics[i_train])
            self.train_poses.append(c2w_mats[i_train])
            self.train_rgb_files.append(np.array(rgb_files)[i_train].tolist())
            num_render = len(i_render)   #3
