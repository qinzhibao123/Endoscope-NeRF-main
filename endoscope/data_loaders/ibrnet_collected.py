...
class IBRNetCollectedDataset(Dataset):
    def __init__(self, args, mode, random_crop=True, **kwargs):
        ...
        self.rectify_inplane_rotation = args.rectify_inplane_rotation  #False
        self.mode = mode  # train / test / validation     #train
        self.num_source_views = args.num_source_views    #10
        self.random_crop = random_crop                  #True

        all_scenes = glob.glob(self.folder_path1 + '*') + glob.glob(self.folder_path2 + '*')   #{list:67}
