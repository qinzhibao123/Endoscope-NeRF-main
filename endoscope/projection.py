...
class Projector():
    ...
    def normalize(self, pixel_locations, h, w):  #h:378, w:504  pixel_locations:(12,426,64,2)
        ...
        normalized_pixel_locations = 2 * pixel_locations / resize_factor - 1.  #(12,426,64,2) [n_views, n_points, 2]
    ...
    def compute_projections(self, xyz, train_cameras):
        ...
        train_intrinsics = train_cameras[:, 2:18].reshape(-1, 4, 4)  #(12,4,4) [n_views, 4, 4]
        train_poses = train_cameras[:, -16:].reshape(-1, 4, 4)  #(12,4,4)  [n_views, 4, 4]      pose
        xyz_h = torch.cat([xyz, torch.ones_like(xyz[..., :1])], dim=-1)  #(426*64,4)  [n_points, 4]
        projections = train_intrinsics.bmm(torch.inverse(train_poses)) \
            .bmm(xyz_h.t()[None, ...].repeat(num_views, 1, 1))  #(12,4,426*64)  [n_views, 4, n_points]
        projections = projections.permute(0, 2, 1)  # [n_views, n_points, 4]
        pixel_locations = projections[..., :2] / torch.clamp(projections[..., 2:3], min=1e-8)  #(12,426*64,2)  [n_views, n_points, 2]
        pixel_locations = torch.clamp(pixel_locations, min=-1e6, max=1e6)  #(12,426*64,2)
        mask = projections[..., 2] > 0   #(12,426*64)  a point is invalid if behind the camera
    ...
    def compute_angle(self, xyz, query_camera, train_cameras):
        ...
        ray2tar_pose = (query_pose[:, :3, 3].unsqueeze(1) - xyz.unsqueeze(0)) #(12,426*64,3)
        ray2tar_pose_q = ray2tar_pose
        # ray2tar_pose /= (torch.norm(ray2tar_pose, dim=-1, keepdim=True) + 1e-6)  #(12,426*64,3)
        ray2tar_pose =ray2tar_pose/(torch.norm(ray2tar_pose, dim=-1, keepdim=True) + 1e-6)
        ray2train_pose = (train_poses[:, :3, 3].unsqueeze(1) - xyz.unsqueeze(0)) #(12,426*64,3)
        ...
        ray_diff = ray2tar_pose - ray2train_pose   #(12,426*64,3)
        ray_diff_norm = torch.norm(ray_diff, dim=-1, keepdim=True)   #(12,426*64,1)

        ray_diff_dot = torch.sum(ray2tar_pose * ray2train_pose, dim=-1, keepdim=True)  #(12,426*64,1)
        ray_diff_direction = ray_diff / torch.clamp(ray_diff_norm, min=1e-6)  #(12,426*64,3)
        ...
        ray_diff = torch.cat([ray_diff_direction, ray_diff_dot, ray_diff_dot*dd], dim=-1)
    ...
    def compute(self,  xyz, query_camera, train_imgs, train_cameras, featmaps):
        ...
        # rgb sampling
        rgbs_sampled = F.grid_sample(train_imgs, normalized_pixel_locations, align_corners=True) #(12,3,426,64)
        rgb_sampled = rgbs_sampled.permute(2, 3, 0, 1)  #(426,64,12,3) [n_rays, n_samples, n_views, 3]

        # deep feature sampling
        feat_sampled = F.grid_sample(featmaps, normalized_pixel_locations, align_corners=True) #(12,32,426,64)
        feat_sampled = feat_sampled.permute(2, 3, 0, 1)  #(426,64,12,32) [n_rays, n_samples, n_views, d]
        rgb_feat_sampled = torch.cat([rgb_sampled, feat_sampled], dim=-1)   #(426,64,12,35)   *****   [n_rays, n_samples, n_views, d+3]

        # mask
        inbound = self.inbound(pixel_locations, h, w)
        ray_diff = self.compute_angle(xyz, query_camera, train_cameras)  #(12,426,64,4)
        ray_diff = ray_diff.permute(1, 2, 0, 3)  #(426,64,12,4)
        mask = (inbound * mask_in_front).float().permute(1, 2, 0)[..., None]   #(426,64,12,1)
